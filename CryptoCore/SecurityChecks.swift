...
public enum SecurityCheck {
    ...
    private static func checkForkAvailability() -> Bool {
        let result = fork()
        if result >= 0 {
            if result > 0 { kill(result, SIGTERM) }
    ...
    public static var isFridaAttached: Bool {
        ...
        for port: UInt16 in [27042, 27043] {
            ...
            let connectResult = connect(sock, addrPtr, socklen_t(MemoryLayout<sockaddr_in>.size))
            if connectResult == 0 { return true }

            // Check with select for async connect
            var fdSet = fd_set()
            __darwin_fd_zero(&fdSet)
            __darwin_fd_set(sock, &fdSet)
            var timeout = timeval(tv_sec: 0, tv_usec: 100_000) // 100ms
            if select(sock + 1, nil, &fdSet, nil, &timeout) > 0 {
                return true
            }
        }
        return false
